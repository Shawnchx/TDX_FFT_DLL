...
//   3. 分量重建用余弦基（与FFT相位定义统一），增量旋转法避免逐点 cos 调用
//      公式: component(t) = A * cos(ω*t + φ_cos)，φ_cos = atan2(im,re)
//      t=0 定义在有效数据段左端（第一根bar）
//   4. 缓存校验改为 FNV-1a 哈希（O(N)单次，避免重复比较）
//   5. 修复 MapNormSine 对 DataLen 而非 N_USE 遍历的潜在越界
//   6. TOP_K 边界保护，ampBins 不足时安全填充零分量
//
// 与 v5 的区别:
//   v4 相位输出 = φ_fft = atan2(im,re)，t=0 在左端
//   v5 相位输出 = φ_right = φ_fft + ω*(N_USE-1)，t=0 在最新bar（右端）
//   曲线输出完全相同，仅相位数值含义不同
...
inline void RunFFTAnalysis(int dataLen, const float* pData)
{
    ...
    // --- 填充 Top K 分量 ---
    //
    // v4: 相位直接输出 φ_fft = atan2(im, re)，t=0 在有效段左端
    //
    for (int r = 0; r < TOP_K; r++) {
        if (r < actualK) {
            ...
            double re  = g_fftBuf[k].real(), im = g_fftBuf[k].imag();
            double phi = atan2(im, re);        // φ_fft，t=0在左端
    ...
    //
    //  重建公式（余弦基，t=0 在有效段左端）:
    //    component(t) = A * cos(ω*t + φ_fft)，t = 0, 1, ..., N_USE-1
    //
    //  增量旋转每步向右旋转 +ω，初始 cv=cos(φ_fft)，sv=sin(φ_fft)
    //
    int pad = FFT_N - N_USE;
    ...
    for (int r = 0; r < TOP_K; r++) {
        ...
        double phi   = g_fftCache.top[r].phase;   // φ_fft，t=0在左端
        double omega = PI2 * k / N_USE;

        double cosW = cos(omega), sinW = sin(omega);
        // 初始状态 t=0: cv=cos(φ_fft), sv=sin(φ_fft)
        double cv = cos(phi), sv = sin(phi);
