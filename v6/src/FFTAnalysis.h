...
//   8. (v6) 取数偏移 shift: pfINb 控制数据窗口起点
//      shift=0: 从最新bar取 N_USE 根（默认，原行为）
//      shift=1: 从第1根前的位置取 N_USE 根（当日不参与，验证比较用）
//      shift=n: 从第n根bar向前取数，范围 0~32
...
struct FFTCache {
    int           dataLen;
    int           N_USE;
    int           shift;                        // 取数偏移：0=最新bar，n=从第n根bar向前取
    ...
    FFTCache() : dataLen(0), N_USE(0), shift(0), inputHash(0), valid(false) {
...
// ============================================================
//  主分析函数
//  shift: 取数偏移，0=最新bar，1~32=从第n根bar向前取
// ============================================================
inline void RunFFTAnalysis(int dataLen, const float* pData, int shift = 0)
{
    ...
    // --- 确定 N_USE（基于去头后的有效数据量）---
    int effectiveLen = dataLen - shift;
    if (effectiveLen < 64) effectiveLen = 64;
    int useLen = (effectiveLen >= FFT_N) ? FFT_N : effectiveLen;
    int N_USE = 64;
    while (N_USE * 2 <= useLen) N_USE *= 2;

    int srcStart = dataLen - N_USE - shift;
    const float* src = pData + srcStart;

    // --- 哈希校验缓存（含shift，不同偏移产生独立cache）---
    uint32_t h = fnv1a_floats(src, N_USE);
    if (g_fftCache.valid &&
        g_fftCache.dataLen == dataLen &&
        g_fftCache.N_USE   == N_USE   &&
        g_fftCache.shift   == shift  &&
...
inline void MapFFTSeqToOutput(int dataLen, float* pfOUT, const float* fftSeq)
{
    ...
    int copyLen = N_USE;
    int fftOff  = 0;
    int dstOff  = kStart > 0 ? kStart : 0;

    // 如果kStart<0（数据不足），跳过fftSeq头部
    if (kStart < 0) {
        fftOff = -kStart;
        copyLen -= fftOff;
        dstOff  = 0;
    ...
    // 限制不超出dataLen
    if (dstOff + copyLen > dataLen)
        copyLen = dataLen - dstOff;

    if (copyLen > 0)
        memcpy(pfOUT + dstOff, fftSeq + pad + fftOff, copyLen * sizeof(float));

    // 末尾剩余清零
    if (dstOff + copyLen < dataLen)
        memset(pfOUT + dstOff + copyLen, 0, (dataLen - dstOff - copyLen) * sizeof(float));
}
