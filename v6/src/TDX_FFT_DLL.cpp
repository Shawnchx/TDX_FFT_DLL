...
//
//  v6 shift参数（公式调用时传参B）:
//    TDXDLL{n}(内部编号, CLOSE, SHIFT, 0)
//    SHIFT 范围 0~32
//      0  = 默认，从最新bar向前取数
//      1  = 忽略当日，从上一个bar取（验证比较用）
//      n  = 从第n根bar位置开始向前取数
...
// 从pfINb提取shift（取整，范围0~32，内部进一步钳位）
#define GET_SHIFT() ((int)pfINb[0])

// 确保FFT已计算（以pfINa作为价格输入，pfINb[0]为shift）
#define ENSURE_FFT() \
    RunFFTAnalysis(DataLen, pfINa, GET_SHIFT()); \
...
static void MapPartialFit(int DataLen, float* pfOUT, int nComp)
{
    ...
    for (int i = 0; i < DataLen; i++) {
        // 只在有效窗口 [kStart, kStart+N_USE) 内输出，前后均清零
        if (i < kStart || i >= kStart + N_USE) {
            pfOUT[i] = 0.0f;
        } else {
            int t = i - kStart;  // 0..N_USE-1
            if (nComp == 0) {
...
// ============================================================
//  归一化正弦输出辅助
//  v6: 考虑 shift 偏移，只归一化有效窗口 [kStart, kStart+N_USE)
// ============================================================
static void MapNormSine(int DataLen, float* pfOUT, int rank)
{
    ...
    int kEnd = kStart + N_USE;
    if (kEnd > DataLen) kEnd = DataLen;
